...
class ViewpointSelector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.atts = [
            AttentionBlock(512, 512, 512, 8, skip_connect=False) for _ in range(2)
        ]
        self.mlps = [
            nn.Sequential(
                nn.Conv1d(512 * 2, 512, 1, 1),
    ...
    def extract_ref_feats(
        self, ref_imgs, ref_poses, object_center, object_vert, is_train=False
    ):
        # get features
        ...
        # get pose embedding
        ref_cam_pts = (
            -ref_poses[:, :3, :3].permute(0, 2, 1) @ ref_poses[:, :3, 3:]
        )  # rfn,3,3 @ rfn,3,1
        ...
        # get rotation
        y = torch.linalg.cross(object_vert, object_forward)
        x = torch.linalg.cross(y, object_vert)
        object_vert = F.normalize(object_vert, dim=0)
        x = F.normalize(x, dim=0)
        y = F.normalize(y, dim=0)
        R = torch.stack([x, y, object_vert], 0)
        ref_cam_pts = ref_cam_pts @ R.T  # rfn,3 @ 3,3
        ref_cam_pts = F.normalize(
            ref_cam_pts, dim=1
        )  # rfn, 3 --> normalized viewpoints here
    ...
    def load_ref_imgs(self, ref_imgs, ref_poses, object_center, object_vert):
        ...
        ref_imgs = torch.from_numpy(
            color_map_forward(ref_imgs).transpose([0, 1, 4, 2, 3])
        ).cuda()  # an,rfn,3,h,w
        ref_poses, object_center, object_vert = (
            torch.from_numpy(ref_poses.astype(np.float32)).cuda(),
    ...
    def select_que_imgs(self, que_imgs):
        ...
        @return:
        """
        que_imgs = torch.from_numpy(
            color_map_forward(que_imgs).transpose([0, 3, 1, 2])
        ).cuda()  # qn,3,h,w
    ...
    def compute_view_point_feats(self, que_imgs):
        ...
        for ref_feats, que_feats, corr_conv in zip(
            ref_feats_list, que_feats_list, self.corr_conv_list
        ):
            ...
            feats_corr = feats_corr.permute(0, 3, 1, 2, 4, 5).reshape(
                qn, f, rfn * an, h, w
            )
            feats_corr_ = corr_conv(feats_corr)
            _, f_, _, h_, w_ = feats_corr_.shape
            corr_feats.append(
                feats_corr_.reshape(qn, f_, rfn, an, h_, w_)
            )  # qn,f_,rfn,an,h_,w_

            # vps score feats
            score_maps = torch.sum(feats_corr, 1)  # qn,rfn*an,h,w
            score_maps_ = (
                score_maps / (torch.max(score_maps.flatten(2), 2)[0][..., None, None])
            )  # qn,rfn*an,h,w
            score_vps = torch.sum(
                score_maps.flatten(2) * score_maps_.flatten(2), 2
            )  # qn,rfn*an
        ...
        corr_feats = self.corr_feats_conv(corr_feats.reshape(qn, f, rfn * an, h, w))[
            ..., 0, 0
        ]  # qn,f,rfn,an
        corr_feats = corr_feats.reshape(qn, corr_feats.shape[1], rfn, an)
        vps_feats = self.vp_norm(
            torch.stack(vps_feats, 1),
        )  # qn,3,rfn,an
    ...
    def forward(self, data):
        ...
        self.extract_ref_feats(
            ref_imgs, ref_poses, object_center, object_vert, is_train
        )
