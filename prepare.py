...
def video2image(input_video, output_dir, interval=30, image_size=640, transpose=False):
    ...
    while success:
        if count % interval == 0:
            ...
            imsave(
                f"{output_dir}/frame%d.jpg" % count, image
...
def prepare_validation_set(
    ref_database_name, que_database_name, ref_split, que_split, estimator_cfg
):
    ...
    save_pickle(
        img_id2det_info,
        f"data/val/det/{que_database_name}/{estimator.detector.cfg['name']}.pkl",
    )
    save_pickle(
        img_id2sel_info,
        f"data/val/sel/{que_database_name}/{estimator.detector.cfg['name']}-{estimator.selector.cfg['name']}.pkl",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transpose", action="store_true", dest="transpose", default=False
    )
    ...
    if args.action == "video2image":
        video2image(
            args.input, args.output, args.frame_inter, args.image_size, args.transpose
        )
    elif args.action == "sfm":
        build_colmap_model_no_pose(
            parse_database_name(args.database_name), args.colmap_path
        )
