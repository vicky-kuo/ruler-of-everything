...
from utils.base_utils import (
    ...
    transformation_scale_2d,
    transformation_compose_2d,
    transformation_crop,
    transformation_rotation_2d,
)
...
def compute_similarity_transform_batch(pts0, pts1):
    ...
    d1 = pts1 - c1[:, None, :]
    scale = np.mean(
        np.linalg.norm(d1, 2, 2, keepdims=True), 1, keepdims=True
...
def ransac_similarity_transform(corr):
    ...
    while True:
        ...
        scale, rotation, offset = compute_similarity_transform_batch(
            seed[:, :, :2], seed[:, :, 2:]
        )  #
    ...
    scale, rotation, offset = compute_similarity_transform_batch(
        inlier_corr[None, :, :2], inlier_corr[None, :, 2:]
    )
...
def compose_similarity_transform(scale, rotation, offset):
    ...
    M = transformation_compose_2d(
        M, np.concatenate([rotation, np.zeros([2, 1])], 1).astype(np.float32)
    )
...
class Gen6DEstimator:
    ...
    def _load_module(cfg):
        ...
        state_dict = torch.load(
            f"data/model/{refiner_cfg['name']}/model_best.pth", weights_only=False
        )
        refiner.load_state_dict(state_dict["network_state_dict"])
        print(
            f"load from {refiner_cfg['name']}/model_best.pth step {state_dict['step']}"
        )
        refiner.cuda().eval()
        return refiner
    ...
    def build(self, database: BaseDatabase, split_type: str):
        ...
        ref_ids = select_reference_img_ids_fps(
            database, ref_ids_all, self.cfg["ref_view_num"]
        )
        ref_imgs, ref_masks, ref_Ks, ref_poses, ref_Hs = normalize_reference_views(
            database, ref_ids, self.cfg["ref_resolution"], 0.05
        )
        ...
        for angle in angles:
            ...
            for rfi in range(rfn):
                ...
                ref_imgs_rot.append(
                    cv2.warpPerspective(
                        database.get_image(ref_ids[rfi]),
                        H_new,
                        (w, h),
        ...
        self.selector.load_ref_imgs(
            ref_imgs_rots, ref_poses, object_center, object_vert
        )
        self.ref_info = {
            "imgs": ref_imgs,
            "ref_imgs": ref_imgs_rots,
            "masks": ref_masks,
            "Ks": ref_Ks,
            "poses": ref_poses,
    ...
    def predict(self, que_img, que_K, pose_init=None):
        ...
        if pose_init is None:
            ...
            que_img_, _ = transformation_crop(
                que_img, position, 1 / scale_r2q, 0, self.cfg["ref_resolution"]
            )  # h,w,3
        ...
        if self.refiner is not None:
            ...
            for k in range(self.cfg["refine_iter"]):
                pose_pr = self.refiner.refine_que_imgs(
