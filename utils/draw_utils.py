...
def draw_correspondence(
    ...
    if max_draw_line_num is not None and matches.shape[0] > max_draw_line_num:
        ...
        if colors is not None and (type(colors) is list or type(colors) is np.ndarray):
    ...
    for mi, m in enumerate(matches):
        ...
        elif type(colors) is list or type(colors) is np.ndarray:
    ...
    if not vert:
        ...
        if h0 < h1:
            img0 = cv2.copyMakeBorder(
                img0, 0, h1 - h0, 0, 0, borderType=cv2.BORDER_CONSTANT, value=0
            )
        if h1 < h0:
            img1 = cv2.copyMakeBorder(
                img1, 0, h0 - h1, 0, 0, borderType=cv2.BORDER_CONSTANT, value=0
            )
        ...
        if w0 < w1:
            img0 = cv2.copyMakeBorder(
                img0, 0, 0, 0, w1 - w0, borderType=cv2.BORDER_CONSTANT, value=0
            )
        if w1 < w0:
            img1 = cv2.copyMakeBorder(
                img1, 0, 0, 0, w0 - w1, borderType=cv2.BORDER_CONSTANT, value=0
            )
...
def draw_bbox(img, bbox, color=None, thickness=2):
    ...
    img = cv2.rectangle(
        img, (left, top), (left + width, top + height), color, thickness=thickness
    )
    return img
